...
import { z } from 'zod';
import createCertificateAction from '@/actions/certificate/createCertificate';
import updateCertificateAction from '@/actions/certificate/updateCertificate';
import deleteCertificateAction from '@/actions/certificate/deleteCertificate';
...
export function CertificateManager({ certificates, onRefresh }: CertificateManagerProps) {
    ...
    const createForm = useForm<CertificateFormData>({
        resolver: zodResolver(certificateSchema) as any,
        defaultValues: { name: '', issuer: '', date: '', language: 'tr', orderNum: 0, isActive: true },
    });
    ...
    const handleCreate = async (data: CertificateFormData) => {
        try {
            ...
            toast({ description: 'Sertifika başarıyla eklendi!' });
            createForm.reset();
            setIsCreateOpen(false);
            onRefresh();
        } catch {
            toast({ variant: 'destructive', description: 'Sertifika eklenirken hata oluştu.' });
        } finally {
            setIsCreating(false);
        }
    };
    ...
    const handleUpdate = async (data: CertificateFormData) => {
        if (!selectedItem) return;
        try {
            setIsUpdating(true);
            await updateCertificateAction({ ...data, id: selectedItem.id });
            toast({ description: 'Sertifika başarıyla güncellendi!' });
            setIsEditOpen(false);
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            toast({ description: 'Sertifika silindi.' });
            onRefresh();
        } catch {
    ...
    const renderFormFields = (form: UseFormReturn<CertificateFormData, any, any>) => (
        <>
            ...
            <FormField control={form.control} name="name" render={({ field }) => (
                <FormItem>
                    <FormLabel>Sertifika Adı</FormLabel>
                    <FormControl><Input placeholder="Sertifika adı" {...field} /></FormControl>
                    <FormMessage />
                </FormItem>
            )} />
            <div className="grid grid-cols-2 gap-4">
                <FormField control={form.control} name="issuer" render={({ field }) => (
                    <FormItem>
                        <FormLabel>Veren Kurum (İsteğe bağlı)</FormLabel>
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-semibold">Sertifika Bilgileri</h3>
                <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                    <DialogTrigger asChild>
                        <Button><Plus className="h-4 w-4 mr-2" />Yeni Sertifika</Button>
                    </DialogTrigger>
                    <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-white dark:bg-dark-card border-none shadow-2xl">
                        <DialogHeader><DialogTitle>Yeni Sertifika Ekle</DialogTitle></DialogHeader>
                        <Form {...createForm}>
            ...
            <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-white dark:bg-dark-card border-none shadow-2xl">
                    <DialogHeader><DialogTitle>Sertifika Düzenle</DialogTitle></DialogHeader>
                    <Form {...editForm}>
