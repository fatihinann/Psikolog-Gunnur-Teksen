...
export async function submitContactForm(formData: z.infer<typeof contactFormSchema>) {
  try {
    ...
    const { error } = await resend.emails.send({
      from: 'Günnur Teksen Web <onboarding@resend.dev>', // Should be a verified domain in production
    ...
    if (error) {
      ...
      // We still return true because it was saved to DB, but maybe log it
    }
